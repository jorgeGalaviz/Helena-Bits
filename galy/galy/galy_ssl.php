...
class GY_SSL{
	...
	public function getSesion(){
		...
		if( $es_form ){
			$query = "select id_usuario, pass_cod, nombre from ap_usuarios"
				. " where usuario = '".$_POST["usuario"]."' and pass_cod = '".sha1(md5($_POST["pass"]))."'";
			$id_user = $this->getIdUser( $query );
			if( $id_user != 0 ){
				$query = "update ap_usuarios set accesos = accesos + 1 where id_usuario = " . $id_user;
	...
	public function setCloseSesion(){
		$id_user = $this->getIdUserCook();
		// Generar Registro
	...
	public function getDatos(){
		...
		if( $id_user != 0 ){
			$query = "select DATE_FORMAT(fec_alta, '%b %Y') as fec_alta, nombre
				from ap_usuarios where id_usuario = $id_user";
			$resultado = $this->db->ejecutar($query);
			if ($usuario = $this->db->getFetchAssoc($resultado) ) {
				$datosDinamicos["nombre"] = $usuario["nombre"];
				$datosDinamicos["fecAlta"] = $usuario["fec_alta"];
			}
	...
	private function getIdUserCook(){
		...
		if( isset($_COOKIE[self::$miniDatos["id_user"]]) && isset($_COOKIE[self::$miniDatos["pass"]]) ){
			if( $_COOKIE[self::$miniDatos["id_user"]] != "" && $_COOKIE[self::$miniDatos["pass"]] != ""  ){
				$query = "select id_usuario, pass_cod, nombre from ap_usuarios"
					. " where id_usuario = '".$this->decrypt($_COOKIE[self::$miniDatos["id_user"]], $this->miniLlave)."' "
